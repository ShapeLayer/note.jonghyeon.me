...
현재 처리하는 정점과 연결된 정점들을 큐에 삽입하고 이후에 큐에서 인출하면, 큐는 선입선출 원리에 따라 시작점에서 가까운 정점을 반환한다. 최초에는 시작점과 연결된 정점들이 큐에 삽입되고, 반복 과정이 진척됨에 따라 점차 그로부터 멀어지는 정점들이 삽입된다.
## 깊이 우선 탐색의 구현
...
## 마무리
BFS와 DFS는 PS에서 쉬운 알고리즘 축에 속한다. 그래서 대부분 BFS, DFS는 다른 알고리즘에 비해 비교적 앞서 접하게 되는 느낌인 것 같다. 하지만 알고리즘이 낯선 입장에서는 이 역시 쉽지 않디. 파트 타임 강사 아르바이트를 하고 있는 학원의 알고리즘 반에서도, 많은 학생들이 이 단계에서 꽤 좌절을 겪는 편이다.
